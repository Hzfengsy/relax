...
private:
 ...
 void VisitLeaf(const Expr& leaf_expr, IndexedForwardGraph::Node* binding_var_node,
   ...
   if (it != graph_.node_map.end()) {
     ...
     LOG(FATAL) << "The leaf Expr is supposed to be defined before";
   }
...
public:
 ...
 void CreateFunction() {
   ...
   for (size_t i = 0; i < bindings_.size() - 1; ++i) {
   ...
   Var output_var = builder_->EmitOutput(VisitExpr(last_binding->value));
   var_remap_[last_binding->var->vid] = output_var;

   // Step 4. Finish constructing the new block.
   BindingBlock new_block = builder_->EndBlock();
   Expr body = builder_->Normalize(SeqExpr({new_block}, output_var));
...
public:
 ...
 IRModule Transform() {
   ...
   builder_->UpdateFunction(main_gv, updated_main_func);
